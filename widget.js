(function() {
    // ၁။ Connection URL ကို ပြင်ဆင်ခြင်း (စမ်းသပ်ရန် 127.0.0.1 ပြောင်းထားသည်)
    const socket = io('http://127.0.0.1:3000'); 
    const OWNER_ID = "69ed2070fb1e9db48fb0fcdd"; 
    const GUEST_ID = 'guest_' + Math.floor(Math.random() * 1000000);

    // ၂။ UI ကို သင့် Website နဲ့ လိုက်ဖက်အောင် Neon Style ပြောင်းလဲခြင်း
    const widgetHTML = `
        <div id="nexus-widget" style="position: fixed; bottom: 20px; right: 20px; z-index: 9999; font-family: 'Outfit', sans-serif;">
            <button id="nexus-launcher" style="width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, #00f2ff, #ff0055); color: white; border: none; cursor: pointer; box-shadow: 0 0 15px #00f2ff; font-size: 24px;">💬</button>
            <div id="nexus-chat-box" style="display: none; width: 320px; height: 450px; background: rgba(10,10,15,0.95); border: 1.5px solid #00f2ff; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); flex-direction: column; position: absolute; bottom: 80px; right: 0; overflow: hidden; backdrop-filter: blur(15px);">
                <div style="background: linear-gradient(90deg, #00f2ff, #ff0055); color: white; padding: 15px; font-weight: bold; font-family: 'Syncopate'; font-size: 12px;">NEXUS CHAT</div>
